...
<div class="bg-white text-white text-center  w-[350px]   shadow-md p-6 rounded-2xl">
	...
	{:else}
		...
		<div class="text-black text-center text-5xl my-5">
			{$vocab_store[item]?.vocab === 'undefined' ? '' : $vocab_store[item]?.vocab}
		</div>
		<div class="flex flex-col justify-center items-center">
			<button
				on:click={() => handleClick('a')}
				class={`bg-yellow text-dark rounded-lg  p-2 w-full text-semibold text-xl`}
				>{$vocab_store[item]?.a}</button
			>

			<button
				on:click={() => handleClick('b')}
				class={`bg-yellow text-dark  rounded-lg  p-2  m-3 w-full text-semibold text-xl`}
